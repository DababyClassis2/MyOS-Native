...
- [x] Scaffold `MyOS-Native` Tauri v2 project
- [x] Establish `src/` (frontend) and `src-tauri/` (backend) separation
- [x] Implement Rust `get_system_health` command (CPU, RAM, disk)
- [x] Create desktop shell — dock icons, top bar, wallpaper
- [x] Implement `open_module` native window spawner
- [x] Connect `AppState` pattern with `Arc<Mutex<T>>`
- [x] Write `ARCHITECTURE.md` v1
...
- [x] **Terminal module**
  - [x] macOS-style title bar with traffic light buttons
  - [x] Rust `exec_command` bridge with command whitelist
  - [x] Command history (up/down arrow)
  - [x] `clear` command support
  - [x] Output line limit (prevent memory overflow from verbose commands)
- [x] **Activity Monitor module**
  - [x] Real-time CPU and RAM charts (2-second poll)
  - [x] Native process list via `sysinfo` crate
  - [x] Process name + PID + RSS memory
- [x] **File Browser module (read-only)**
  - [x] Rust `list_dir` command
  - [x] Grid view with file/folder icons
  - [x] Sandboxed root paths (no traversal above allowed roots)
